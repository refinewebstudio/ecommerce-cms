// components/storyblok/Testimonial.tsx
import { storyblokEditable } from '@storyblok/react/rsc';
import {richTextResolver} from '@storyblok/richtext';

interface TestimonialProps {
  blok: {
    _uid: string;
    component: string;
    name?: string;
    role?: string;
    company?: string;
    content?: any; // Rich text content
    avatar?: {
      filename: string;
      alt?: string;
    };
    rating?: number;
  };
}

export default function Testimonial({ blok }: TestimonialProps) {
  const { render } = richTextResolver()
  return (
    <div {...storyblokEditable(blok)} className="rounded-lg bg-white p-8 shadow-sm">
      {/* Star Rating */}
      {blok.rating && (
        <div className="mb-4 flex">
          {[...Array(5)].map((_, i) => (
            <svg
              key={i}
              className={`h-5 w-5 ${
                i < blok.rating! ? 'text-yellow-400' : 'text-gray-300'
              }`}
              fill="currentColor"
              viewBox="0 0 20 20"
            >
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
            </svg>
          ))}
        </div>
      )}

      {/* Testimonial Content */}
      {blok.content && (
        <div 
          className="mb-6 text-gray-600"
          dangerouslySetInnerHTML={{ 
            __html: render(blok.content) as string
          }}
        />
      )}

      {/* Customer Info */}
      <div className="flex items-center">
        {/* Avatar */}
        {blok.avatar ? (
          <img
            src={blok.avatar.filename || null}
            alt={blok.avatar.alt || blok.name || 'Customer avatar'}
            className="h-10 w-10 rounded-full object-cover"
          />
        ) : blok.name ? (
          // Fallback: Show initials if no avatar
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gray-200 text-sm font-medium text-gray-700">
            {blok.name.split(' ').map(n => n[0]).join('').toUpperCase()}
          </div>
        ) : null}

        {/* Name and Role */}
        <div className={blok.avatar || blok.name ? 'ml-4' : ''}>
          {blok.name && (
            <p className="text-sm font-medium text-gray-900">{blok.name}</p>
          )}
          {(blok.role || blok.company) && (
            <p className="text-sm text-gray-500">
              {blok.role}
              {blok.role && blok.company && ', '}
              {blok.company}
            </p>
          )}
        </div>
      </div>
    </div>
  );
}

